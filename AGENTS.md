...
Source of the raw menu: `menu vacanze di branco.csv` (not in the repo), provided by the user. Columns: `Piatto`, then up to 8 pairs of `Ingrediente N` / `Dosi ingrediente N`.
...
- Doses given in the original CSV were kept only when coherent with the group; they scale with the equivalent-portion base too, for consistency.
...
- `Insalata di riso`: original CSV doses were a single/family portion (riso 250gr), not for the group. Recomputed for the full group.
- `Frittata di zucchine`: eggs were missing from the CSV; added (~21) with a note.
...
- The CSV has **no Venerdì** (jumps Giovedì → Sabato); kept as-is, flagged with a `note` on the Sabato day.
- Dishes with no ingredients in the CSV (`Panini`, `Verdure grigliate`, `Frutta`) carry estimated quantities with a "non specificato" note.
